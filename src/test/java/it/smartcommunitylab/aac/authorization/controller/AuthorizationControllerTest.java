...
public class AuthorizationControllerTest {
	...
	public void addRootChild() throws Exception {
		...
		RequestBuilder request = MockMvcRequestBuilders.post("/authorization/schema")
	...
	public void addChildAuthorizationNode() throws Exception {
		ObjectMapper jsonMapper = new ObjectMapper();
		AuthorizationNode node = new AuthorizationNode("A");
		RequestBuilder request = MockMvcRequestBuilders.post("/authorization/schema/parent-qname")
	...
	public void getSchema() throws Exception {
		RequestBuilder request = MockMvcRequestBuilders.get("/authorization/schema/qname-node");
		mockMvc.perform(request).andExpect(MockMvcResultMatchers.status().is2xxSuccessful());
	}

	@Test
	public void removeAuthorization() throws Exception {
		RequestBuilder request = MockMvcRequestBuilders.delete("/authorization/my-auth");
		mockMvc.perform(request).andExpect(MockMvcResultMatchers.status().is2xxSuccessful());
	}
	...
	public void insertAuthorization() throws Exception {
		ObjectMapper jsonMapper = new ObjectMapper();
		Resource resource = new Resource("A", Arrays.asList(new AuthorizationNodeValue("A", "a", "a_value")));
		Authorization auth = new Authorization(new AuthorizationUser("subject", "type"), "action", resource,
				new AuthorizationUser("entity", "type"));
		RequestBuilder request = MockMvcRequestBuilders.post("/authorization")
	...
	public void validateAuthorization() throws Exception {
		...
		Authorization auth = new Authorization(new AuthorizationUser("subject", "type"), "action", resource,
				new AuthorizationUser("entity", "type"));
		RequestBuilder request = MockMvcRequestBuilders.post("/authorization/validate")
				.content(jsonMapper.writeValueAsString(auth)).contentType(MediaType.APPLICATION_JSON);
		mockMvc.perform(request).andExpect(MockMvcResultMatchers.status().is2xxSuccessful())
	...
	public void validateResource() throws Exception {
		...
		ObjectMapper jsonMapper = new ObjectMapper();
		RequestBuilder request = MockMvcRequestBuilders.post("/authorization/schema/validate")

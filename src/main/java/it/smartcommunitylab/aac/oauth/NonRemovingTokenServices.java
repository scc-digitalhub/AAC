...
public class NonRemovingTokenServices extends DefaultTokenServices {
    ...
    private OAuth2AccessToken refreshWithRepeat(String refreshTokenValue, TokenRequest request, boolean repeat) {
        ...
        synchronized (this.refreshLock) {
            ...
            // no need to check for user approval, on removal also refresh token are
            // invalidated
            // if (accessToken == null) {
            ...
            if (accessToken != null) {
                ...
                // note that request could have an empty scope, which means keep the authorized
                // scopes
            ...
            try {
                // NOTE this will remove all existing access tokens associated with the current
                // one
                ...
                // // do retry: it may be the case of race condition so retry the operation but
                // only once
        ...
        if (existingAccessToken != null) {
            if (existingAccessToken.isExpired()) {
                ...
                if (existingAccessToken.getRefreshToken() != null) {
                    ...
                    // The token store could remove the refresh token when the access token is
                    // removed, but we want to
                    // be sure...
                ...
                OAuth2AccessToken accessToken = tokenEnhancer != null
                        ? tokenEnhancer.enhance(existingAccessToken, authentication)
                        : existingAccessToken;
...
//			        throw new AuthenticationServiceException("Token generation error");
                ...
                if (!existingAccessToken.getValue().equals(accessToken.getValue())) {
                    logger.debug("existing access token for authentication " + authentication.getName()
                            + " needs to be updated");
        ...
        // Clients might be holding existing refresh tokens, so we re-use it in the case
        // that the old access token
        // expired.
        if (refreshToken == null) {
...
//           throw new AuthenticationServiceException("Token generation error");
    ...
    private ExpiringOAuth2RefreshToken createRefreshToken(OAuth2Authentication authentication) {
        ...
        logger.trace("create refresh token for " + authentication.getOAuth2Request().getClientId() + " with validity "
    ...
    private OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
        ...
        logger.trace("create access token for " + authentication.getOAuth2Request().getClientId() + " with validity "
                + String.valueOf(validitySeconds));
        if (!authentication.isClientOnly()) {
            token.setExpiration(new Date(System.currentTimeMillis()
                    + (getUserAccessTokenValiditySeconds(authentication.getOAuth2Request(), validitySeconds) * 1000L)));
    ...
    protected int getUserAccessTokenValiditySeconds(OAuth2Request clientAuth, int defaultValiditySeconds) {
        if (clientAuth.getScope().contains(Config.SCOPE_OPERATION_CONFIRMED))
            return SCOPE_OPERATION_CONFIRMED_DURATION;

...
public class OAuth2ClientJwtAssertionAuthenticationProvider extends ClientAuthenticationProvider
    ...
    public OAuth2ClientJwtAssertionAuthenticationProvider(OAuth2ClientDetailsService clientDetailsService,
            String... audience) {
    ...
    public ClientAuthentication authenticate(Authentication authentication) throws AuthenticationException {
        Assert.isInstanceOf(OAuth2ClientJwtAssertionAuthenticationToken.class, authentication,
                "Only OAuth2ClientJwtAssertionAuthenticationToken is supported");

        OAuth2ClientJwtAssertionAuthenticationToken authRequest = (OAuth2ClientJwtAssertionAuthenticationToken) authentication;
        ...
        try {
            ...
            if (jwtDecoder == null) {
                this.logger.debug("Failed to authenticate since auth method is unsupported: "
            ...
            if (assertResult.hasErrors()) {
                this.logger
                        .debug("Failed to authenticate due to assertion errors: "
                                + String.valueOf(assertResult.getErrors()));
                throw new BadCredentialsException("invalid authentication");
            }

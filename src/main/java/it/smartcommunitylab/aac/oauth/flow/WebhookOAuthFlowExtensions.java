...
//	private SymmetricKeyJWTValidatorCacheService symmetricCacheService;
    ...
    @Override
    public void onAfterApproval(AuthorizationRequest authorizationRequest, Authentication userAuthentication)
        ...
        String hook = getAfterApprovalWebhook(client);
        if (StringUtils.hasText(hook)) {
            try {
                ...
                ResponseEntity<String> result = restTemplate.exchange(url.toString(), HttpMethod.GET, entity,
                        String.class);
    ...
    protected String extractToken(AuthorizationRequest authorizationRequest, Authentication userAuthentication,
            ClientDetailsEntity client) throws FlowExecutionException {
        ...
        Map<String, Object> clientClaims = claimManager.getClientClaims(client.getClientId(),
        ...
        JWT jwt = jwtService.buildAndSignJWT(client, claims);

        logger.trace("signed jwt from service :" + jwt.serialize());
...
//		}
     ...
     * ClientDetails
     */
    private String getAfterApprovalWebhook(ClientDetailsEntity client) {
        return (String) client.getAdditionalInformation().get(Config.CLIENT_INFO_WEBHOOK_AFTER_APPROVAL);
    }

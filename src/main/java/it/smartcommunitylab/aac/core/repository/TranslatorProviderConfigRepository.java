...
public class TranslatorProviderConfigRepository<U extends AbstractProviderConfig<?, ?>, C extends AbstractProviderConfig<?, ?>>
    ...
    public Collection<C> findAll() {
        return externalRepository.findAll().stream()
    ...
    public Collection<C> findByRealm(String realm) {
        return externalRepository.findByRealm(realm).stream()
                .map(e -> getConverter().convert(e))
                .filter(e -> e != null)
                .collect(Collectors.toList());
    }

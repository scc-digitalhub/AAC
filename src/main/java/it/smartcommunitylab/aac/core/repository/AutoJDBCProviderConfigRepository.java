...
public class AutoJDBCProviderConfigRepository<U extends AbstractProviderConfig<?, ?>>
    ...
    private static final String DEFAULT_CREATE_TABLE_STATEMENT = "CREATE TABLE IF NOT EXISTS provider_config (type VARCHAR(256), provider_id VARCHAR(256),  realm VARCHAR(256), config BLOB, PRIMARY KEY (provider_id, type));";
    private static final String DEFAULT_SELECT_STATEMENT = "select config from provider_config where provider_id = ? and type = ?";
    private static final String DEFAULT_FIND_ALL_STATEMENT = "select config from provider_config where type = ?";
    private static final String DEFAULT_FIND_BY_REALM_STATEMENT = "select config from provider_config where type = ? and realm = ?";
    private static final String DEFAULT_INSERT_STATEMENT = "insert into provider_config (type, provider_id, realm, config) values (?, ?, ?, ?)";
    private static final String DEFAULT_UPDATE_STATEMENT = "update provider_config set config = ?  where provider_id = ? and type = ?";
    private static final String DEFAULT_DELETE_STATEMENT = "delete from provider_config where provider_id = ? and type = ?";

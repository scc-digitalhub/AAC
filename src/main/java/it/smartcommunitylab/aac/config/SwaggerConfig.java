...
public class SwaggerConfig {
    ...
    public Docket apiCore() {
        return new Docket(DocumentationType.SWAGGER_2)
                .groupName("AAC API")
                .apiInfo(apiInfo("AAC Core api", "API for administration of realms"))
                .select()
                .apis(RequestHandlerSelectors.basePackage("it.smartcommunitylab.aac.api"))
//                .paths(PathSelectors.regex("/api/services.*"))
                .build()
                .tags(new Tag("AAC API", "Management api"))
                .securitySchemes(Arrays.asList(
                        securitySchemeApp(aacServiceMgmt())))
                .securityContexts(Arrays.asList(
                        SecurityContext.builder()
                                .securityReferences(
                                        Arrays.asList(new SecurityReference("application", aacServiceMgmt())))
                                .build()));
    }

    private AuthorizationScope[] aacServiceMgmt() {
        AuthorizationScope[] scopes = {
                new AuthorizationScope("api.core",
                        "Core api"),
        };
        return scopes;
    }
    ...
    private SecurityScheme securitySchemeUser(AuthorizationScope[] scopes) {
        GrantType acGrantType = new AuthorizationCodeGrantBuilder()
                ...
                .tokenRequestEndpoint(
                        new TokenRequestEndpoint(AUTH_SERVER + "/eauth/authorize", CLIENT_ID, CLIENT_SECRET))
                .build();

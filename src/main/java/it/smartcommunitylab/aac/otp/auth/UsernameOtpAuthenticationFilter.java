 ...
 * Intercepts login requests sent to the OTP identity authority.
 */

public class UsernameOtpAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
     ...
     * @param otpService               OTP credentials service.
     * @param registrationRepository   Provider configuration repository.
     * @param filterProcessingUrl      Filter processing URL.
     * @param authenticationEntryPoint Authentication error entry point.
     */
    ...
    /**
     * Handles Magic Link based authentication.
     * Extracts username and code from request parameters and attempts
     * authentication.
     */
    ...
    /**
     * Handles standard authentication flow via POST.
     */
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        String providerId = requestMatcher.matcher(request).getVariables().get("registrationId");
        OtpIdentityProviderConfig providerConfig = registrationRepository.findByProviderId(providerId);
        ...
        if (!StringUtils.hasText(username) || !StringUtils.hasText(otp)) {
            AuthenticationException e = new BadCredentialsException("invalid user or otp");
            throw new InternalAuthenticationException(null, username, otp, "otp", e, e.getMessage());
        }

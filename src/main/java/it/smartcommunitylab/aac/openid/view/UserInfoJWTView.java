...
//	private SymmetricKeyJWTValidatorCacheService symmetricCacheService;
    ...
    protected void writeOut(Map<String, Object> json, Map<String, Object> model,
        ...
        try {
            ...
            logger.trace("userInfo claims " + claims.toString());

            JWT token = jwtService.buildAndSignJWT(client, claims);

            logger.trace("userInfo token result " + token.toString());

            Writer out = response.getWriter();
            out.write(token.serialize());

            // DEPRECATED, build JWT locally
